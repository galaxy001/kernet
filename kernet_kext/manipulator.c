...
void kn_fulfill_ip_ranges()
{
    ...
    // *.wordpress.com
    kn_append_readable_ip_range_entry_default_ports("74.200.243.251", 17, ip_range_kernet_2);
    ...
    for(i=sizeof(prefix)/sizeof(prefix[0])-1; i>=0; i-=1) {
        ...
        prefix[i][len++] = '0';
        prefix[i][len++] = 0;
        kn_append_readable_ip_range_entry_80_port(prefix[i], 24, ip_range_kernet_2);
    }
}
